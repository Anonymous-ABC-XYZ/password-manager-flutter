...
void main() {
  ...
  group('Session Expiration', () {
    test(
      'session expires if more than 30 days passed since initial login',
      () async {
        final initialLogin = DateTime.now().subtract(const Duration(days: 31));

        when(
          () => mockStorage.read(key: 'google_initial_login_time'),
        ).thenAnswer((_) async => initialLogin.toIso8601String());
        when(
          () => mockStorage.read(key: 'token_last_refresh'),
        ).thenAnswer((_) async => initialLogin.toIso8601String());
    ...
    test(
      'session remains valid if less than 30 days passed since initial login',
      () async {
  ...
  group('Sign Out', () {
    test('signOut sets expiration message if expired is true', () async {
      ...
      expect(
        ...
        equals('Access session has expired or been revoked'),
      );
    });
  ...
  group('Manual Refresh', () {
    test(
      'refreshToken returns false and signs out if session is expired',
      () async {
